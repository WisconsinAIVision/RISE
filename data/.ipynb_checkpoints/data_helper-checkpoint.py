...
from data.JigsawLoader import JigsawIADataset, JigsawTestIADataset
...
vlcs_datasets = ["CALTECH", "LABELME", "PASCAL", "SUN", "VOS"]
pacs_datasets = ["art_painting", "cartoon", "photo", "sketch"]
office_datasets = ["amazon", "dslr", "webcam"]
terra_datasets = ["location_100", "location_38", "location_43", "locatoin_46"]
available_datasets = office_datasets + pacs_datasets + vlcs_datasets + terra_datasets
...
def get_train_dataloader(args, patches):
    ...
    for dname in dataset_list:
        ...
        train_dataset = JigsawIADataset(name_train, labels_train, patches=patches, img_transformer=img_transformer,
                                      tile_transformer=tile_transformer, jig_classes=args.jigsaw_n_classes, bias_whole_image=args.bias_whole_image)
        if limit:
            train_dataset = Subset(train_dataset, limit)
        datasets.append(train_dataset)
        val_datasets.append(
            JigsawTestIADataset(name_val, labels_val, img_transformer=get_val_transformer(args),
...
def get_val_dataloader(args, patches=False):
    ...
    val_dataset = JigsawTestIADataset(names, labels, patches=patches, img_transformer=img_tr, jig_classes=args.jigsaw_n_classes)
