...
class ResNetV2(nn.Module):
    ...
    def __init__(
        ...
        #     use_conv=True,
        # )
        self.global_pool, _ = _create_pool(self.num_features, self.num_classes, pool_type=global_pool,
                                           use_conv=False, input_fmt='NCHW')
        self.projector = nn.Linear(self.num_features, 512)  # ---
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        if self.drop_rate:
            x_p = F.dropout(x_p, p=float(self.drop_rate), training=self.training)
        return x_p, self.fc(x_p)
    ...
    def forward(self, x):
        ...
        # return x
        x = self.forward_features(x)
        x_p, logits = self.forward_head(x)
        return x_p, logits
